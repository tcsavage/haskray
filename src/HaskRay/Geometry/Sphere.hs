...
mkSphereShape s m = Shape { intersect = intersect s m, center = center s, boundingBox = boundingBox s, mapTexture = mapTexture s, emissive = isEmissive m }
    where
        intersect (Sphere center rad) material ray@(Ray origin dir) = is
            where
                ...
                intersection_at_time t = Intersection (normal_at_time t) (positionAtTime ray t) ray
