...
pixelGrid randoms (View cameraPos lookingAt viewUp fov) width height = map (map transform) grid
    where
        ...
        adj t c = (c / t) - 0.5
        grid = map (map (\(x, y) -> (tfovx * adj width x, tfovy * adj height y)) . (sampleizePixel randoms)) (genGrid width height)
...
genRandomOffsets samp rand = pair rands
    where
        ...
        pair _ = error "HaskRay.Projection.getRandomOffsets: Non-even list (for some reason)."
